document.addEventListener('DOMContentLoaded', () => {
  ...
  function setActiveLink() {
    ...
    // Special case: Check if we're below the cambios section but not yet at the very bottom
      ...
      if (cambiosSection && scrollPosition >= cambiosSection.offsetTop + cambiosSection.clientHeight - 200) {
        currentSection = '#footer';
      }
      // Regular section detection
      else {
        // Special case: If we're at the top of the page (above servicios), activate inicio
        const serviciosSection = document.getElementById('servicios');
        if (serviciosSection && scrollPosition < serviciosSection.offsetTop - 200) {
